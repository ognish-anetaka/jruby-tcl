...
class InterpTest < Test::Unit::TestCase
  ...
  def teardown
    @interp.dispose if defined?(@interp.dispose)
  end
  
  def test_load_from_file
    vars, procs = @interp.vars, @interp.procs
    @interp.dispose if @interp and defined?(@interp.dispose)
...
#    EOF
    # for jacl
    str = @interp.to_tcl
    assert str.include?("set a 0")
#    assert str.include?("array set b {a 1 b 2}")
    assert str.include?("array set b {b 2 a 1}")
    assert str.include?("proc c args return")
    assert str.include?("proc d {a {b 0}} {return $b}")
    assert str.include?("e {} {}")
  end
